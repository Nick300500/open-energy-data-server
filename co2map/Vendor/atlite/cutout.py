...
from Vendor.atlite.convert import (
...
from Vendor.atlite.data import available_features, cutout_prepare
from Vendor.atlite.datasets import modules as datamodules
from Vendor.atlite.gis import (
    compute_availabilitymatrix,
    compute_indicatormatrix,
    compute_intersectionmatrix,
    get_coords,
)
from Vendor.atlite.utils import CachedAttribute
...
class Cutout:
    ...
    def layout_from_capacity_list(self, data, col="Capacity"):
        ...
        >>> pv.plot()
        """
        # Snap each entry to its nearest grid coordinate with plain numpy
        # instead of a dask-backed vectorized `.sel(..., "nearest")`. For
        # capacity lists with hundreds of thousands of rows (e.g. per-plant
        # solar data), dask's fancy-indexing/shuffle machinery builds a task
        # graph whose memory cost scales with the number of indices rather
        # than the (tiny) grid size, which reliably caused MemoryError here.
        # Since the cutout grid is rectilinear, nearest-neighbor lookup on x
        # and y can be done independently via searchsorted, giving the same
        # per-entry grid coordinate that xarray's "nearest" would.
        def _nearest_grid_value(grid_coords, points):
            grid_sorted = np.sort(np.asarray(grid_coords))
            ...
            return grid_sorted[idx]

        nearest_x = _nearest_grid_value(self.data.x.values, data.x.values)
        nearest_y = _nearest_grid_value(self.data.y.values, data.y.values)

        data = (
            data.assign(x=nearest_x, y=nearest_y)
            .groupby(["y", "x"])[col]
            .sum()
        )

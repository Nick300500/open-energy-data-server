...
def handles_labels_for(color_series, kind="circle", kw_list=None, **kw):
    color_series = (
        pd.Series(color_series) if isinstance(color_series, dict) else color_series
    )
    ...
    if kind == "circle":
        handles = [
            Line2D([], [], c=c, marker=".", linestyle="None", markersize=20, **k, **kw)
    ...
    #                    for c, k in zip(colors, kw_list)]
        ...
        handles = [
            Line2D((0, 1), (0.5, 0.5), color=c, **k, **kw)
            for c, k in zip(colors, kw_list)
        ]
...
def load_regions(n, onshore=True):
    ...
    return gpd.read_file(
        "models/" + prepend + "_".join(name.split("_")[:3]) + ".geojson"
    ).set_index("name")

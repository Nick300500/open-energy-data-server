...
from numpy import conj, real, sign
...
from .linalg import dedup_axis, diag, dot, inv
from .utils import (
    as_sparse,
    check_branch_comps,
    check_passive_branch_comps,
    check_snapshots,
    lower,
    upper,
)
...
def average_participation(
    ...
    **kwargs,
    ...
    res = A.to_dataset(name="peer_to_peer").assign_attrs(
        method="Average Participation", aggregated=aggregated
    )
...
def marginal_participation(
    ...
    **kwargs,
    ...
    P = (q * (upper(P) + A) + (1 - q) * (lower(P) - B) + s * (P + C - D)).assign_coords(
        snapshot=snapshot
    )
    ...
    res = Dataset(
        {"virtual_injection_pattern": P, "virtual_flow_pattern": F}, attrs=attrs
    )
    if round is not None:
        res = res.round(round).assign_attrs(res.attrs)
...
def equivalent_bilateral_exchanges(
    n,
    snapshot=None,
    branch_components=None,
    aggregated=True,
    ...
    **kwargs,
    ...
    F = (H @ P).rename(injection_pattern="bus")
    attrs = {
        "method": "Eqivalent Bilateral Exchanges",
        "q": q,
...
def zbus_transmission(
    n, snapshot=None, linear=True, downstream=None, branch_components=None, **kwargs
):
    r"""
    ...
    vip = K.dot(vif.rename(bus="injection_pattern"), "branch").transpose(
        ..., "bus", "injection_pattern"
    )
    ds = Dataset(
        {"virtual_flow_pattern": vif, "virtual_injection_pattern": vip},
...
def with_and_without_transit(n, snapshots=None, branch_components=None):
    ...
    def regional_with_and_withtout_flow(region):
        ...
        region_branches = branches.query(
            "bus0 in @in_region_buses " "or bus1 in @in_region_buses"
        )
        buses_i = (
            pd.Index(region_branches.bus0.unique())
        ...
        p_wo = (
            xr.concat([p.loc[in_region_buses], netImOrEx], dim="bus")
            .reindex(bus=buses_i)
            .fillna(0)
        )
        ...
        if "Link" in branch_components:
            H = xr.concat(
                (PTDF(n, branch_components, snapshot=sn) for sn in snapshots),
                dim="snapshot",
        ...
        res = Dataset(
            {
                "flow_with_transit": f.sel(branch=branches_i),
                "flow_without_transit": f_wo,
            }
        ).assign_coords(country=region)
        return res.assign(transit_flow=res.flow_with_transit - res.flow_without_transit)

    progress = ProgressBar()
    flows = xr.concat(
        (regional_with_and_withtout_flow(r) for r in progress(regions)), dim="country"
    )
    ...
    loss = (
        xr.concat(
            (
                ...
                else flows.sel(component=c)
                * DataArray(n.df(c).efficiency, dims="branch_i")
                for c in comps
            ),
            dim=comps,
        )
        .stack(branch=["component", "branch_i"])
        .rename_vars(
...
def marginal_welfare_contribution(
    n, snapshots=None, formulation="kirchhoff", return_networks=False
):
    ...
    def profit_by_gen(n):
        price_by_generator = n.buses_t.marginal_price.reindex(
            columns=n.generators.bus
        ).set_axis(n.generators.index, axis=1, inplace=False)
        revenue = price_by_generator * n.generators_t.p
        cost = n.generators_t.p.multiply(n.generators.marginal_cost, axis=1)
        return (revenue - cost).rename_axis("profit").rename_axis("generator", axis=1)

    snapshots = check_snapshots(snapshots, n)
    n.lopf(snapshots, solver_name="gurobi", formulation=formulation)
    m = n.model
    ...
    for line in n.lines.index:
        ...
        # extract results
        extract_optimisation_results(n_temp, snapshots, formulation="kirchhoff")
    ...
    return pd.Series(networks).rename_axis("removed line").rename("Network")
...
def flow_allocation(
    ...
    **kwargs,
    ...
    if all(c.pnl.p0.empty for c in n.iterate_components(n.branch_components)):
        raise ValueError(
            "Flows are not given by the network, "
    ...
    if method not in _func_dict.keys():
        raise (
            ValueError(
                "Method not implemented, please choose one out of"

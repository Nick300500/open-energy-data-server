...
from .grid import Incidence, power_demand, power_production, self_consumption
from .utils import check_dataset, is_sparse, obj_if_acc, upper
...
def virtual_patterns(ds, n, q=0.5):
    ...
    vfp = q * da.sum("sink").rename(source="bus") + (1 - q) * da.sum("source").rename(
        sink="bus"
    )
    ...
    virtuals = xr.Dataset(
        {
            "virtual_flow_pattern": vfp.T.assign_attrs(attrs),
...
def peer_to_peer(ds, n, aggregated=None, q=None):
    ...
    if not aggregated:
        assert "q" in vip.attrs, (
            'No shift parameter "q" in attributes' ' of "virtual_injection_pattern".'
        )
        q = vip.attrs["q"]
        p2p = vip.rename(injection_pattern="sink", bus="source")
        s = (1 - q) * power_demand(n, ds.snapshot) - q * power_production(
            n, ds.snapshot
        )
    else:
        p2p = upper(
            vip.rename(injection_pattern="sink", bus="source")
            - vip.rename(injection_pattern="source", bus="sink")
        )
        s = self_consumption(n, ds.snapshot)

    s["bus"] = pd.MultiIndex.from_tuples(
        [(b, b) for b in s["bus"].values], names=["source", "sink"]
    )
...
def vip_to_p2p(ds, n, direct=False):
    warnings.warn(
        "The function 'vip_to_p2p' is deprecated, use " "'peer_to_peer' instead.",
        DeprecationWarning,
        2,
    )

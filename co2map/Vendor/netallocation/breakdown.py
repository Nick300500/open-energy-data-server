...
from .grid import network_injection, power_demand, power_production
from .utils import (
    as_sparse,
    check_carriers,
    check_dataset,
...
def expand_by_source_type(ds, n, chunksize=None, dim="source"):
    ...
    share = (
        (prod / prod.sum("carrier")).rename(bus=dim, carrier="source_carrier").fillna(0)
    )
...
def expand_by_sink_type(ds, n, chunksize=None, dim="sink"):
    ...
    share = (demand / demand.sum("carrier")).rename(bus=dim, carrier="sink_carrier")
    assert dim in ds.dims, f"Dimension {dim} not present in Dataset"
    expand = ds[[k for k in ds if dim in ds[k].dims]]
    if is_sparse(expand):
        share = as_sparse(share.fillna(0))

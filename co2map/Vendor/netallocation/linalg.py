...
def inv(df, pre_clean=False):
    if isinstance(df.data, COO):
        if pre_clean:
            data = df.data
            assert df.ndim <= 2, (
                "Maximally two dimension supported for " "sparse inverse"
            )
            mask = np.isin(np.arange(data.shape[0]), data.coords[0]) & np.isin(
                np.arange(data.shape[1]), data.coords[1]
            )
            subdf = df[mask][:, mask]
            return (
                DataArray(as_coo(sp_inv(subdf.data.tocsc())), subdf.T.coords)
    ...
    if pre_clean:
        ...
        subdf = df.loc[mask]
        return DataArray(
            np.linalg.inv(subdf), subdf.T.coords, subdf.dims[::-1]
...
def dedup_axis(da, newdims):
    ...
    assert not isinstance(oldindex, pd.MultiIndex), (
        "Multiindex expanding not " "supported"
    )
    return DataArray(
        da.data,
        {
            newdims[0]: oldindex.rename(newdims[0]),
            newdims[1]: oldindex.rename(newdims[1]),
        },
        newdims,
    )
...
def _dot_single(df, df2):
    ...
    return DataArray(
        res,
        {dim0: df.coords.indexes[dim0], dim1: df2.coords.indexes[dim1]},
        (dim0, dim1),
    )
...
def null(df):
    ...
    dim = df.dims[-1]
    return DataArray(
        pd.DataFrame(sp.linalg.null_space(df), index=df.get_index(dim)),

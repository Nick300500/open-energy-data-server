...
from .linalg import diag, dot, null, pinv
from .utils import (
    check_branch_comps,
...
def Cycles(n, branches_i=None):
    ...
    """
    branches = pd.concat(
        {c: n.df(c)[["bus0", "bus1"]] for c in sorted(n.branch_components)}
    ...
    mgraph = nx.from_pandas_edgelist(
        branches, edge_attr=True, create_using=nx.MultiGraph
    )
    ...
    return DataArray(
        C.todense(),
        {"branch": branches_i, "cycle": range(C.shape[1])},
        ("branch", "cycle"),
    )
...
def impedance(
    ...
    if linear:
        z = pd.concat(
            {
                c: n.df(c)[x].where(
                    n.df(c).bus0.map(n.buses.carrier) == "AC", n.df(c)[r]
                )
                for c in comps
            }
        )
    ...
    if snapshot is None:
        logger.warn(
            'Link in argument "branch_components", but no '
    ...
    C = Cycles(n, branches_i[abs(f).values > 1e-8]).reindex(
        branch=branches_i, fill_value=0
    )
    ...
    if not C_mix.size:
        ...
        d = {"branch": "Link"}
        omega = -dot(
            pinv(dot(C_mix.loc["Link"].T, diag(f.loc["Link"]))),
...
def admittance(n, branch_components=None, snapshot=None, pu_system=True, linear=True):
    """
...
def series_shunt_admittance(n, pu_system=True, branch_components=None):
    ...
    shunt = pd.concat(
        {
            c.name: c.df[g].fillna(0) + 1.0j * c.df[b].fillna(0)
...
def Ybus(n, branch_components=None, snapshot=None, pu_system=True, linear=True):
    ...
    branch_components = check_passive_branch_comps(branch_components, n)
    K = Incidence(n, branch_components)
    y = admittance(n, branch_components, snapshot, pu_system=pu_system, linear=linear)
    Y = dot(K, diag(y), K.T)
    if linear:
        return Y
    else:
        if not pu_system:
            raise NotImplementedError(
                "Non per unit system " "for non-linear Ybus matrix not " "implemented"
            )
        ...
        def get_Ybus(sub):
            sub.calculate_PTDF()
            sub.calculate_Y()
            return pd.DataFrame(sub.Y.todense(), index=sub.buses_o, columns=sub.buses_o)

        return n.sub_networks.obj.apply(get_Ybus)


def Zbus(n, branch_components=None, snapshot=None, pu_system=True, linear=True):
    """
...
def voltage(n, snapshots=None, linear=True, pu_system=True):
    ...
    #        v = np.exp(- 1.j * n.buses_t.v_ang).T[snapshots]
        ...
        v = (n.buses_t.v_mag_pu * np.exp(1.0j * n.buses_t.v_ang)).loc[snapshots]
    ...
    if isinstance(snapshots, (list, pd.Index)):
        ...
        return DataArray(v.reindex(n.buses.index), dims="bus").assign_coords(
            snapshot=snapshots
        )


def network_flow(n, snapshots=None, branch_components=None, ingoing=True, linear=True):
    """
    ...
    f = pd.concat(
        {b: n.pnl(b)[p].loc[snapshots, n.df(b).index] for b in comps}, axis=axis
    )
    if not linear:
...
def _one_port_attr(n, snapshots=None, attr="p", comps=None):
    ...
    gen = (
        reindex_by_bus_carrier(n.pnl(c)[attr].loc[snapshots] * n.df(c).sign, c, n)
        for c in comps
    )
...
def power_production(n, snapshots=None, per_carrier=False, update=False):
    ...
    if "p_plus" not in n.buses_t or update:
        ...
        n.buses_t["p_plus"] = prod.sel(
            carrier=(prod >= 1e-8).any(["snapshot", "bus"])
        ).clip(min=0)
...
def power_demand(n, snapshots=None, per_carrier=False, update=False):
    ...
    if "p_minus" not in n.buses_t or update:
        ...
        n.buses_t["p_minus"] = (
            (-demand)
            .sel(carrier=(demand <= -1e-8).any(["snapshot", "bus"]))
            .clip(min=0)
        )

...
def get_branches_i(n, branch_components=None):
    ...
    return pd.concat({c: n.df(c)[[]] for c in branch_components}).index.rename(
        ["component", "branch_i"]
    )
...
def get_ext_branches_i(n, branch_components=None):
    ...
    branch_components = check_branch_comps(branch_components, n)
    return pd.Index(
        sum(([(c, i) for i in get_extendable_i(n, c)] for c in branch_components), [])
    )
...
def get_non_ext_branches_i(n, branch_components=None):
    ...
    return pd.Index(
        sum(
            ([(c, i) for i in get_non_extendable_i(n, c)] for c in branch_components),
            [],
        )
    )
...
def get_ext_one_ports_i(n, per_carrier=True):
    ...
    if per_carrier:
        return pd.MultiIndex.from_frame(
            pd.concat(
                n.df(c).loc[get_extendable_i(n, c), ["bus", "carrier"]] for c in comps
            )
        )
    return pd.Index(sum(([(c, i) for i in get_extendable_i(n, c)] for c in comps), []))
...
def get_non_ext_one_ports_i(n, per_carrier=True):
    ...
    if per_carrier:
        return pd.MultiIndex.from_frame(
            pd.concat(
                n.df(c).loc[get_non_extendable_i(n, c), ["bus", "carrier"]]
                for c in comps
            )
        )
    return pd.Index(
        sum(([(c, i) for i in get_non_extendable_i(n, c)] for c in comps), [])
    )
...
def get_ext_one_ports_b(n):
    check_carriers(n)
    gen = [
        reindex_by_bus_carrier(n.df(c)[attr + "_extendable"], c, n)
...
def get_ext_branches_b(n):
    ds = pd.concat(
        {
            c: n.df(c)[attr + "_extendable"]
            for c, attr in nominal_attrs.items()
            if c in sorted(n.branch_components)
        },
...
def split_one_ports(ds, n, dim="bus"):
    ...
    d = {"ext": ds.where(ext_b).fillna(0), "fix": ds.where(~ext_b).fillna(0)}
...
def as_sparse(ds):
    ...
    def func(data):
        return COO(data) if not isinstance(data, COO) else data
...
def as_dense(ds):
    ...
    def func(data):
        return data.todense() if isinstance(data, COO) else data
...
def reindex_by_bus_carrier(df, c, n):
    ...
    if isinstance(df, pd.DataFrame):
        ...
        return xr.DataArray(df, dims=["snapshot", "dim_1"]).unstack(
            "dim_1", fill_value=0
        )
    else:
...
def get_as_dense_by_bus_carrier(n, attr, comps=None, snapshots=None):
    ...
    return xr.concat(
        (
            reindex_by_bus_carrier(get_switchable_as_dense(n, c, attr, snapshots), c, n)
            for c in comps
        ),
        dim="carrier",
    ).reindex(bus=buses_i, fill_value=0)
...
def check_duplicated_carrier(n):
    ...
    dupl = (
        pd.Series(
            {
                c: n.df(c).carrier.unique()
                for c in n.one_port_components
                if "carrier" in n.df(c)
            }
        )
        .explode()[lambda ds: ds.duplicated(keep=False)]
        .dropna()
    )
    assert dupl.empty, (
        f"The carrier name(s) {dupl.to_list()} appear in more "

...
def allocate_one_port_operational_cost(ds, n, snapshots=None, dim="source"):
    ...
    mc = get_as_dense_by_bus_carrier(n, "marginal_cost", comps, snapshots).rename(
        bus=dim, carrier="source_carrier"
    )
    attr = {"payer": dim, "allocation": "one_port_operational_cost"}
    return (mc * ds).assign_attrs(attr)


def allocate_co2_cost(
    ds, n, dim="source", co2_constr_name=None, co2_attr="co2_emissions", co2_price=None
):
    """
    ...
    check_carriers(n)
    ds = expand_by_source_type(ds, n, dim=dim)
    ep = nodal_co2_price(n, snapshots, co2_attr, co2_constr_name, co2_price).rename(
        bus=dim, carrier="source_carrier"
    )
...
def allocate_one_port_investment_cost(ds, n, dim="source", proportional=False):
    ...
    attr = nominal_attrs
    nom_opt = concat(
        [reindex_by_bus_carrier(n.df(c)[attr[c] + "_opt"], c, n) for c in comps],
        dim="carrier",
    )
    cap_cost = concat(
        (reindex_by_bus_carrier(n.df(c).capital_cost, c, n) for c in comps),
        dim="carrier",
    )
    ...
    return (investment_cost.reindex_like(normed, fill_value=0) * normed).assign_attrs(
        attrs
    )
...
def allocate_branch_operational_cost(ds, n):
    ...
    snapshots = check_snapshots(ds.snapshot, n)
    check_carriers(n)
    branchcost_pu = pd.concat(
        [get_as_dense(n, "Link", "marginal_cost", snapshots)],
        keys=["Link"],
        axis=1,
...
def allocate_branch_investment_cost(ds, n, proportional=False):
    ...
    if not proportional:
        mu = pd.concat(
            {c: n.pnl(c).mu_upper - n.pnl(c).mu_lower for c in comps},
            axis=1,
            names=names,
        )
    ...
    investment_cost = pd.concat(
        {c: n.df(c).eval(f"capital_cost * {attr}") for c, attr in nom_attr.items()},
        names=names,
    )
...
def cycle_constraint_cost(n, snapshots=None):
    ...
    for sub in n.sub_networks.obj:
        coords = {
            "branch": sub.branches().index.rename(("component", "branch_i")),
...
def congestion_revenue(n, snapshots=None, split=False):
    ...
    """
    cr = (
        -locational_market_price_diff(n, snapshots).transpose()
        * network_flow(n, snapshots)
        * snapshot_weightings(n, snapshots)
    )
...
def nodal_demand_cost(n, snapshots=None):
    ...
    """
    snapshots = check_snapshots(snapshots, n)
    return (energy_demand(n, snapshots) * locational_market_price(n, snapshots)).rename(
        "nodal_demand_cost"
    )


def nodal_co2_price(
    n, snapshots=None, co2_attr="co2_emissions", co2_constr_name=None, price=None
):
    """
    ...
    if price is None:
        if co2_constr_name is None:
            ...
            co2_constr_name = con_i[
                con_i.str.contains("CO2", case=False)
...
def nodal_co2_cost(
    n, snapshots=None, co2_attr="co2_emissions", co2_constr_name=None, split=False
):
    """
...
def allocate_revenue(
    n, snapshots=None, chunksize=None, with_cycle_prices=False, **kwargs
):
    """
    ...
    d = dict(
        ...
        source_carrier="receiver_carrier",
    )

    res = Dataset(
        {
            "dispatch_revenue": p2p * lmp.rename(bus="source"),
...
def allocate_cost(n, snapshots=None, method="ap", chunksize=None, **kwargs):
    ...
    def rename(da):
        return da.rename({k: v for k, v in d.items() if k in da.dims})

    res = Dataset(
        {
            da.attrs["allocation"]: rename(da)
            for da in [op_one_port, co2_one_port, inv_one_port, op_branch, inv_branch]
            if da.sum() != 0
        }
    )
    return res.sel(snapshot=res.snapshot[0]) if was_timestep else res

...
from .utils import as_dense, as_sparse, is_sparse, lower, upper
...
def local_consumption(ds):
    ...
    return xr.concat(
        (ds.peer_to_peer.sel(source=b, sink=b) for b in buses), dim="source"
    )
...
def nonlocal_consumption(ds):
    ...
    buses = ds.get_index("source").rename("bus")
    return xr.concat(
        (ds.peer_to_peer.sel(source=b).drop_sel(sink=b) for b in buses), dim="source"
    )
...
def consider_branch_extension_on_flow(ds, n, chunksize=30):
    orig_branch_cap = xr.DataArray(
        dims="branch",
        data=pd.concat(
            [n.lines.s_nom_min, n.links.p_nom_min],
            keys=["Line", "Link"],
    ...
    with ProgressBar(minimum=2.0):
        extension_flow = (
            extension_flow.chunk(chunk) * abs(flow_shares).chunk(chunk)
        ).compute()
        within_cap_flow = (
            within_cap_flow.chunk(chunk) * abs(flow_shares).chunk(chunk)
        ).compute()

    return xr.Dataset(
        {"on_extension": extension_flow, "on_original_cap": within_cap_flow},
        attrs={
            "type": "Extension flow allocation" f'with method: {ds.attrs["method"]}'
        },
    )
